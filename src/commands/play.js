    ...
    exec: async (msg, args) => {
        ...
        try {
            ...
            if (loadType === "PLAYLIST_LOADED") {
                ...
                msg.channel.send(util.embed().setDescription(`<a:certo:865931553628356629> Loaded \`${tracks.length}\` tracks from **${name}**.`));
            } else {
                const track = tracks[0];
                track.requester = msg.author;
                music.queue.push(track);
                if (music.player && music.player.playing)
                    msg.channel.send(util.embed().setDescription(`<a:certo:865931553628356629> **${track.info.title}** added to the queue.`));
            }
