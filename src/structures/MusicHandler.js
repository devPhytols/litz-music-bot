    ...
    async join(voice) {
        ...
        this.player
            .on("start", () => {
                ...
                if (this.textChannel) this.textChannel.send(util.embed().setDescription(`<a:z_oblackworld:865932097687781396> Now playing **${this.current.info.title}**.`));
            })
            .on("end", (data) => {
                ...
                if (!this.queue.length) {
                    ...
                    if (this.textChannel) this.textChannel.send(util.embed().setDescription("<a:certo:865931553628356629> Queue is empty. Leaving voice channel.."));
                    this.reset();
                    return;
                }
                this.start();
